...
You should see logs showing `MatrixAdapter` starts sync loop, `Scheduler` starts (if enabled), and no immediate errors from `matrix-nio`.

## 2. Send a ping from the tester account

**Option A:** Type `hello hestia` in the test room via Element.

**Option B:** Use `matrix-commander`:

```bash
matrix-commander -r "!your-room:server" -m "hello hestia"
```

**Option C:** Use `scripts/matrix_test_send.py`:
...
Press **Ctrl-C** in the terminal running `hestia matrix`. The bot will cancel the sync loop and exit cleanly. If running under systemd in the runtime tree, use `sudo systemctl stop hestia-agent@$USER`.
