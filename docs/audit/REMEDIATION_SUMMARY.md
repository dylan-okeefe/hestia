...
10. `fix(web+tools)` — SEC-002/023 login recipient allowlisting, roster minimization (SEC-004), debug_login guard (SEC-006), topic IDOR (SEC-007), scheduler admin parity (BUG-082), fs tools atomic/bounded (PERF-011a/b), terminal env-allowlist/output-cap/timeout-clamp (SEC-015)
