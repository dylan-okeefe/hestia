...
- Name: config.spec.ts >> each trust preset card shows description and bullets
- Location: playwright/config.spec.ts:22:1
...
Locator: locator('text=Maximum safety. Every tool requires explicit confirmation.')
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('text=Maximum safety. Every tool requires explicit confirmation.')
