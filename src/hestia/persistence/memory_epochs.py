...
async def _compile_and_set_memory_epoch(
    ...
    if app.topic_store is not None:
        topic_ids = await app.topic_store.get_conversation_topic_ids(session.id)

    epoch = await app.epoch_compiler.compile(session, topic_ids=topic_ids)
