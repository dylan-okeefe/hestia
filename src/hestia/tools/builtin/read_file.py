...
# Default read_file for backward compatibility (no sandboxing)
# This will be replaced by the factory-created tool in cli.py
async def _default_read_file(path: str, max_bytes: int = 1_000_000) -> str:
    """Default read_file without sandboxing (for backward compatibility)."""
    p = Path(path)
    if not p.exists():
        return f"File not found: {path}"
    if not p.is_file():
