...
class StyleProfileBuilder:
    ...
    async def _active_users(self, since: datetime) -> list[tuple[str, str]]:
        sql = sa.text(
            """
            SELECT DISTINCT platform, platform_user
            FROM sessions
    ...
    async def _compute_preferred_length(
        ...
        sql = sa.text(
            """
            SELECT t.session_id, t.started_at, t.completion_tokens
            FROM traces t
            JOIN sessions s ON t.session_id = s.id
            WHERE s.platform = :platform
              AND s.platform_user = :platform_user
        ...
        async with self._db.engine.connect() as conn:
            traces = (await conn.execute(
                sql,
                {"platform": platform, "platform_user": platform_user, "since": since.isoformat()},
            )).fetchall()
        if not traces:
            return None
        values: list[int] = []
    ...
    async def _has_length_feedback(self, session_id: str, after: datetime | str) -> bool:
        ...
        async with self._db.engine.connect() as conn:
            rows = (
                await conn.execute(sql, {"session_id": session_id, "after": after_str})
            ).fetchall()
        for row in rows:
